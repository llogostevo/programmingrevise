...
import { CheckCircle2, CircleAlert, LoaderCircle, Play, Send, Terminal } from "lucide-react";
...
function HintControls({ hints, used, onUse }: { hints: [string, string, string]; used: number; onUse: () => void }) {
  return (
    <div className="rounded-xl border border-amber-200 bg-warning-soft/65 p-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          <p className="text-sm font-semibold text-foreground">Need a nudge?</p>
          <p className="text-xs text-muted-foreground">Hints become more specific one step at a time.</p>
        </div>
        {used < 3 ? (
      ...
      {used > 0 ? (
        <ol className="mt-3 space-y-2 text-sm leading-6">
          {hints.slice(0, used).map((hint, index) => (
            <li key={hint} className="flex gap-2"><span className="font-mono text-xs font-bold text-warning">{index + 1}</span><span>{hint}</span></li>
          ))}
        </ol>
      ) : null}
    </div>
...
function Feedback({ result }: { result: CheckResult }) {
  ...
  return (
    <div className={cn("rounded-xl border p-4", correct ? "border-emerald-200 bg-success-soft" : partial ? "border-amber-200 bg-warning-soft" : "border-rose-200 bg-rose-50")} role="status" aria-live="polite">
      <div className="flex gap-3">
        ...
        <div>
          <p className="font-semibold capitalize">{result.status.replaceAll("-", " ")}</p>
          <p className="mt-1 text-sm leading-6">{result.message}</p>
          {result.executionError ? <pre className="mt-3 overflow-x-auto rounded-lg bg-slate-950 p-3 font-mono text-xs text-rose-200">{result.executionError}</pre> : null}
          {result.total > 1 ? <p className="mt-2 text-xs font-medium text-muted-foreground">Tests passed: {result.passed} of {result.total}</p> : null}
        </div>
      </div>
    </div>
  );
  ...
  return (
    <article className="space-y-5 rounded-2xl border bg-card p-4 shadow-sm sm:p-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <Badge variant={exercise.id.endsWith("guided") ? "info" : "success"}>{exercise.id.endsWith("guided") ? "Guided build" : "Independent build"}</Badge>
      ...
      <div className="grid gap-3 rounded-xl bg-muted/65 p-4 text-sm sm:grid-cols-2">
        <div>
          <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Example input</p>
          <p className="mt-1 font-mono">{exercise.visibleExample.input.length ? exercise.visibleExample.input.join(" · ") : "No input"}</p>
        </div>
        <div>
          <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Expected output</p>
          <pre className="mt-1 whitespace-pre-wrap font-mono">{exercise.visibleExample.expectedOutput}</pre>
        </div>
      </div>
      ...
      <div className="overflow-hidden rounded-xl border border-slate-700 bg-slate-950">
        <div className="flex items-center justify-between border-b border-slate-800 px-4 py-2 text-xs font-semibold text-slate-400">
          ...
          <span>Python 3 · runs in your browser</span>
        </div>
        <CodeMirror
          value={code}
          height="260px"
      ...
      <div className="flex flex-wrap gap-3">
        ...
        {loading ? <span className="self-center text-xs text-muted-foreground">{loading === "run" ? "Starting Python…" : "Running test cases…"}</span> : null}
      </div>

      <div className="overflow-hidden rounded-xl border bg-slate-950 text-slate-100">
        <div className="flex items-center gap-2 border-b border-slate-800 px-4 py-2 text-xs font-semibold text-slate-400"><Terminal className="size-3.5" /> Console</div>
        <pre className="min-h-16 whitespace-pre-wrap p-4 font-mono text-sm leading-6">{runError ? <span className="text-rose-300">{runError}</span> : consoleOutput || <span className="text-slate-500">Run your code to see its output here.</span>}</pre>
      </div>

      <HintControls hints={exercise.hints} used={hintsUsed} onUse={() => setHintsUsed((value) => Math.min(3, value + 1))} />
